...
while 1
    ...
    if InputLengthCorrect >= 1
        for i = 1:InputLengthCorrect
            if wordLength ~= correctGuesses % if word is not yet guessed but a letter is correctly guessed
                fprintf('You guessed the correct Letter!\n');
            else % if word length matches the number of correct guesses/if word is correctly guessed               
        ...
        if wrongGuesses ~= 6 % if user has guessed letter incorrectly and still has not made 6 wrong guesses
            fprintf('You guessed the incorrect letter.\n');
            attemptsLeft = attemptsLeft - 1; % update attemptsLeft
            fprintf('You have %d attempts remaining.\n', attemptsLeft);
            fprintf('%s\n',emptyVec)
        else    % if user has lost and has lost all attempts
            fprintf('%s\n',emptyVec);
